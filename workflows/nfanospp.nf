...
WorkflowNfanospp.initialise(params, log)
...
if (params.dada_table) { ch_dada_table = file(params.dada_table) } else { exit 1, 'DADA2 table tsv not specified!' }
...
//
include { ANOSPP } from '../subworkflows/local/anospp'
...
workflow NFANOSPP {
    ...
    // MODULE: MultiQC
    //
    // workflow_summary    = WorkflowNfanospp.paramsSummaryMultiqc(workflow, summary_params)
    // ch_workflow_summary = Channel.value(workflow_summary)

    // methods_description    = WorkflowNfanospp.methodsDescriptionText(workflow, ch_multiqc_custom_methods_description)
